package gitpractice;

public class Bhavesh {

    public static void main(String[] args) {
        System.out.println("lets rock");
        System.out.println("Hey!! Man");
        System.out.println("HIp Hip Hoorey We gonna start appying the jobs now");
        System.out.println("I will do group study all the questions everyday");
        System.out.println("Practice, --------");
        System.out.println("Practice, --------");
        System.out.println("Practice, --------");
        System.out.println("Practice, --------");
        System.out.println("Practice, --------");
        System.out.println("Practice, --------");
        System.out.println("lets rock");
        System.out.println("lets rock");
        System.out.println("lets rock");
        System.out.println("lets rock");
        System.out.println("lets rock");
        System.out.println("lets rock");
        System.out.println("lets rock");
        System.out.println("adding more code");
        System.out.println("Ayan is causing conflict X_X");
        System.out.println("Ayan was here");
        System.out.println("You still don't know me X_X");
        System.out.println("adding another line");
        System.out.println("adding more cod1");
        System.out.println("adding more cod1");
        System.out.println("adding more cod1");
        System.out.println("adding more cod1");
        System.out.println("git practice");
        System.out.println("group study");




    }
}
